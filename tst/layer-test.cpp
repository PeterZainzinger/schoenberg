...
TEST(Layer, layer_activation) {
    ...
    TYPE_EVENTS output;
    auto r = test_run_layer(TYPE_EVENTS({
                                                {1, "F"},
                                                {2, "F"}
                                        }), empty_items);
    ...
    r = test_run_layer(TYPE_EVENTS({{0, "F"}}), empty_items, false);
...
TEST(Layer, layer_deactivation) {
    ...
    auto res = test_run_layer(input, empty_items, false);
...
TEST(Layer, with_basic_prefix) {
    ...
    auto res = test_run_layer(input, output);
...
TEST(Layer, asdf_test) {
    ...
    auto r = test_run_layer(input, input, false);
    auto strokes = effective_keystrokes(r.second);
